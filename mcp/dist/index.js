...
async function handleTool(name, args) {
    switch (name) {
        ...
        // -----------------------------------------------------------------------
        case 'add_character': {
            const { project_id, name: charName, description, visual_description, age, gender } = args;
        ...
        // -----------------------------------------------------------------------
        case 'add_location': {
            const { project_id, name: locName, type, visual_description, lighting, mood } = args;
            const { data, error } = await supabase
                .from('locations')
                .insert({
                project_id,
        ...
        // -----------------------------------------------------------------------
        case 'add_prop': {
            const { project_id, name: propName, type, visual_description } = args;
            const { data, error } = await supabase
                .from('props')
                .insert({
                project_id,
                ...
                .single();
            if (error)
                throw new Error(`Failed to add prop: ${error.message}`);
        ...
        case 'delete_prop': {
            ...
            if (error)
                throw new Error(`Failed to delete prop: ${error.message}`);
            return { success: true };
        }
        // -----------------------------------------------------------------------
        // SCENES
        // -----------------------------------------------------------------------
        case 'add_scene': {
            const { project_id, scene_number, int_ext, location, time_of_day, description } = args;
            ...
            const { data, error } = await supabase
                ...
                .single();
            if (error)
                throw new Error(`Failed to add scene: ${error.message}`);
        ...
        // -----------------------------------------------------------------------
        case 'add_dialogue': {
            const { scene_id, character_name, content, parenthetical, extension } = args;
            // Get next sort order
            const { data: existing } = await supabase
            ...
            const { data, error } = await supabase
                ...
                .single();
            if (error)
                throw new Error(`Failed to add dialogue: ${error.message}`);
            return { success: true, element: data };
        }
        case 'add_action': {
            const { scene_id, content } = args;
            const { data: existing } = await supabase
                .from('script_elements')
                .select('sort_order')
                .eq('scene_id', scene_id)
        ...
        // -----------------------------------------------------------------------
        case 'add_shot': {
            const { scene_id, description, shot_type, camera_angle, camera_movement, duration } = args;
            const { data: existing } = await supabase
                .from('shots')
                .select('shot_number')
                .eq('scene_id', scene_id)
            ...
            const { data, error } = await supabase
                ...
                .single();
            if (error)
                throw new Error(`Failed to add shot: ${error.message}`);
            return { success: true, shot: data };
        }
        case 'update_shot': {
            const { shot_id, ...updates } = args;
            const { data, error } = await supabase
                .from('shots')
                .update(updates)
                .eq('id', shot_id)
        ...
        case 'get_full_script': {
            ...
            const { data: elements, error: elementsError } = await supabase
                ...
                .order('sort_order');
            if (elementsError)
                throw new Error(`Failed to get elements: ${elementsError.message}`);
