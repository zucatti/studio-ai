...
// NOTE: SSE has been removed. Use /api/projects/[projectId]/queue-quick-shot for async generation.
...
// NOTE: The streaming handler has been removed.
// Use /api/projects/[projectId]/queue-quick-shot for async generation with BullMQ.
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    // SSE streaming has been removed - use queue-quick-shot endpoint instead
    if (stream) {
      return NextResponse.json(
        {
          error: 'Streaming not supported',
          message: 'Use /api/projects/[projectId]/queue-quick-shot for async generation',
        },
        { status: 400 }
      );
    }
