...
// GET: Check if assembly is needed (for clip sequences - scene_id IS NULL)
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  // Get sequence - for clips, scene_id is NULL
...
// POST: Queue sequence assembly job (for clip sequences - scene_id IS NULL)
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Get sequence with title - for clips, scene_id is NULL
    const { data: sequence } = await supabase
      .from('sequences')
      .select('id, title, assembled_plan_hash')
      .eq('id', sequenceId)
      .eq('project_id', projectId)
      .is('scene_id', null)
      .single();
    ...
    const { data: job, error: jobError } = await supabase
      ...
      .insert({
        ...
        input_data: {
          ...
          entityType: 'clip', // Mark as clip sequence
        },
    ...
    if (jobError || !job) {
      console.error('[ClipSequenceAssemble] Failed to create job:', jobError);
      return NextResponse.json(
    ...
    // Note: shortId is null for clip sequences
    try {
      await enqueueFFmpeg({
        userId,
        jobId: job.id,
        ...
        projectId,
        shortId: undefined, // No shortId for clip sequences
        sequenceId,
        shotIds,
        planHash: currentHash,
      });
      console.log(`[ClipSequenceAssemble] Job ${job.id} enqueued for sequence ${sequenceId}`);
    } catch (queueError) {
      console.error('[ClipSequenceAssemble] Failed to enqueue:', queueError);
    ...
    console.error('[ClipSequenceAssemble] Unexpected error:', error);
    return NextResponse.json(
