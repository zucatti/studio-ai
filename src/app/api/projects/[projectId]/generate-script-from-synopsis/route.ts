    ...
    async start(controller) {
      ...
      try {
        ...
        if (scenesError || !scenes || scenes.length === 0) {
          ...
          // Log API usage
          logClaudeUsage({
            operation: 'generate-synopsis-from-brainstorming',
            model: 'claude-sonnet-4-20250514',
            inputTokens: synopsisMessage.usage?.input_tokens || 0,
            outputTokens: synopsisMessage.usage?.output_tokens || 0,
            projectId,
          }).catch(console.error);
        ...
        for (let sceneIndex = 0; sceneIndex < scenes.length; sceneIndex++) {
          ...
          // Log API usage
          logClaudeUsage({
            operation: 'generate-shots-from-scene',
            model: 'claude-sonnet-4-20250514',
