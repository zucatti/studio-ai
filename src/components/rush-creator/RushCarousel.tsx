...
export function RushCarousel() {
  ...
  // Combine pending jobs and media into a single array
  // Pending jobs appear first (newest at index 0)
    ...
    <div className="absolute inset-0 flex items-center overflow-hidden">
      ...
      >
        {items.map((item, index) => {
          ...
          const key = item.type === 'pending' ? `pending-${item.job.jobId}` : `media-${item.media.id}`;
